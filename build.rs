...
fn main() {
    ...
    let cache_dir = match env::var_os("GMP_MPFR_SYS_CACHE") {
        ...
        None => dirs::cache_dir().map(|c| c.join("gmpmee-mpfr-sys")),
    };
    ...
    link_dir(&src_dir.join(GMPMEE_DIR), &env.build_dir.join("gmpmee-src"));
    ...
    //process_gmp_header(&gmp_ah.1, &out_dir.join("gmpmee_h.rs"));
    write_link_info(&env, workaround_47048);
}
...
fn save_cache(
    ...
    let (ref a, ref h) = *gmp_ah;
    ok = ok && copy_file(a, &check_dir.join("libgmpmee.a")).is_ok();
    ok = ok && copy_file(h, &check_dir.join("gmpmee.h")).is_ok();
...
fn load_cache(
    ...
    for (version_dir, version_patch) in cache_dirs {
        for req_check in req_checks {
            ...
            let (ref a, ref h) = *gmp_ah;
            ok = ok && copy_file(&check_dir.join("libgmpmee.a"), a).is_ok();
            ok = ok && copy_file(&check_dir.join("gmpmee.h"), h).is_ok();
            if ok {
...
fn should_save_cache(env: &Environment, mpfr: bool, mpc: bool) -> bool {
    ...
    for (version_dir, _) in cache_dirs {
        for req_check in req_checks {
            ...
            ok = ok && check_dir.join("libgmpmee.a").is_file();
            ok = ok && check_dir.join("gmpmee.h").is_file();
            if ok {
                return false;
            }
        }
...
/*fn process_gmp_header(header: &Path, out_file: &Path) {
    ...
    let limb_bits = limb_bits.expect("Cannot determine GMP_LIMB_BITS from gmpmee.h");
    println!("cargo:limb_bits={}", limb_bits);

    let nail_bits = nail_bits.expect("Cannot determine GMP_NAIL_BITS from gmpmee.h");
    if nail_bits > 0 {
        println!("cargo:rustc-cfg=nails");
    }

    let long_long_limb = long_long_limb.expect("Cannot determine _LONG_LONG_LIMB from gmpmee.h");
    ...
    let cc = cc.expect("Cannot determine __GMP_CC from gmpmee.h");
    let cflags = cflags.expect("Cannot determine __GMP_CFLAGS from gmpmee.h");
    let content = format!(
        concat!(
...
fn write_link_info(env: &Environment, workaround_47048: Workaround47048) {
